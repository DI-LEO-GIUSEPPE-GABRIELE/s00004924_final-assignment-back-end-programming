...
import study_project.demo.entities.PassengerAircraft;
import study_project.demo.entities.CargoAircraft;
import study_project.demo.repositories.AircraftRepository;

@RestController
@RequestMapping("/aircrafts")
public class AircraftController {
    private final AircraftRepository aircrafts;
    public AircraftController(AircraftRepository aircrafts) { this.aircrafts = aircrafts; }

    @GetMapping
    public Page<Aircraft> list(Pageable pageable) { return aircrafts.findAll(pageable); }
    ...
    public ResponseEntity<?> delete(@PathVariable UUID id) {
        if (!aircrafts.existsById(id)) return ResponseEntity.notFound().build();
